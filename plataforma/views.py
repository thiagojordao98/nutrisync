...
def refeicao(request, id_paciente):
    ...
    if request.method == "POST":
        ...
        if not titulo:
            messages.add_message(request, constants.ERROR, 'O campo Título não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
        
        if not horario:
            messages.add_message(request, constants.ERROR, 'O campo Horário não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')

        if not carboidratos:
            messages.add_message(request, constants.ERROR, 'O campo Carboidratos não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
        
        if not proteinas:
            messages.add_message(request, constants.ERROR, 'O campo Proteínas não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
        
        if not gorduras:
            messages.add_message(request, constants.ERROR, 'O campo Gorduras não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
...
def opcao(request, id_paciente):
    if request.method == "POST":
        ...
        if not id_refeicao:
            messages.add_message(request, constants.ERROR, 'O campo Refeição não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
        
        if not imagem:
            messages.add_message(request, constants.ERROR, 'O campo Imagem não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
        
        if not descricao:
            messages.add_message(request, constants.ERROR, 'O campo Descrição não pode estar vazio.')
            return redirect(f'/plano_alimentar/{id_paciente}')
